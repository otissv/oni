...
table_layout_registers_node_id_and_kind :: proc(t: ^testing.T) {
	with_widget_env(t, proc(t: ^testing.T) {
		...
		Table(
			{config = {id = "table-1", width = set.Width(f32(80)), height = set.Height(f32(32))}},
		)
...
table_unmount_skips_layout_node :: proc(t: ^testing.T) {
	with_widget_env(t, proc(t: ^testing.T) {
		...
		Table({
			config = {id = "table-gone", width = set.Width(f32(40)), height = set.Height(f32(20))},
			unmount = true,
...
table_config_override_preserves_kind :: proc(t: ^testing.T) {
	with_widget_env(t, proc(t: ^testing.T) {
		props := Table_Props {
			config = {id = "table-styled", background = set.Background(o.Color.PRIMARY)},
		}
		frame := Table_State{}
