...
*/
Table_Head_Config :: o.Widget_Config

/*
Table widget per-frame frame_state merged with its fully resolved style config.
*/
Table_Head_State :: o.Widget_Merged_State(o.Widget_Frame_State, o.Resolved_Widget_Config)

/*
Table widget event snapshot with frame_state and optional input metadata.
*/
Table_Head_Event :: o.Widget_Event(Table_Head_State)


/*
Table widget props: config overrides, child callback, and input event handlers.
*/
Table_Head_Props :: struct {
	config:                       Table_Head_Config,
	child:                        proc(frame_state: Table_Head_State),
	unmount:                      bool,
	can_interactive_during_mount: bool,
	on_mount:                     proc(frame_state: Table_Head_State) -> o.Mount,
	on_unmount:                   proc(frame_state: Table_Head_State) -> o.Mount,
	on_focus:                     proc(event: Table_Head_Event),
	on_blur:                      proc(event: Table_Head_Event),
	on_mouse_enter:               proc(event: Table_Head_Event),
	on_mouse_leave:               proc(event: Table_Head_Event),
	on_mouse_pressed:             proc(event: Table_Head_Event),
	on_mouse_down:                proc(event: Table_Head_Event),
	on_mouse_released:            proc(event: Table_Head_Event),
	on_mouse_move:                proc(event: Table_Head_Event),
	on_click:                     proc(event: Table_Head_Event),
	on_contextmenu:               proc(event: Table_Head_Event),
	on_key_pressed:               proc(event: Table_Head_Event),
	on_key_down:                  proc(event: Table_Head_Event),
	on_key_released:              proc(event: Table_Head_Event),
}
...
*/
@(private)
table_head_theme_base :: proc(frame_state: ^Table_Head_State) -> Table_Head_Config {
...
*/
Table_Head :: proc(props: Table_Head_Props) {
	cfg := props.config
	...
	event := widget_refresh_merged(props, &frame_state, table_head_theme_base)
	style_fp := widget_style_interaction_fp(&frame_state)
	config := frame_state.config
	child := props.child
	handlers := widget_lifecycle_handlers(props, Table_Head_State)
	...
	event, _ = widget_refresh_merged_if_interaction_changed(props, &frame_state, table_head_theme_base, style_fp)
	config = frame_state.config
